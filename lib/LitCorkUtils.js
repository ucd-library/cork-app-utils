...
const LitCorkUtils = subclass =>
  class EventMixin extends subclass {
     ...
     * @method _initLogger
     * 
     * @description Initialize the logger for this element.  You can use this
     * method to set the logger name to something other than the nodeName.
     * 
    ...
    _eb_init() {
      ...
      this.logger.debug(this.nodeName, 'ready and connected to DOM, attaching event listeners', this.bind);
      
      for( var key in this.bind ) {
        if( !this[this.bind[key]] ) {
          this.logger.warn(`${this.nodeName} could not bind event ${key} to ${this.bind[key]}`);
          continue;
        }
    ...
    _eb_init_fn(eventName) {
      ...
      this._eb_handlers[eventName] = (...args) => {
        ...
        if( !this.listening ) {
          this.logger.debug(this.nodeName, 'ignoring', eventName, 'event, element not listening');
          return;
        }

        this.logger.debug(this.nodeName, 'received event', eventName, ', triggering function:', this.bind[eventName]);
    ...
    disconnectedCallback() {
      ...
      this.logger.debug(this.nodeName, 'disconnected from DOM, removing event listeners');
      ...
      for( var key in this.bind ) {
        ...
        if( EventBus.listenerCount(key) !== count-1 ) {
          this.logger.warn(this.nodeName, 'On element detach, failed to remove event listener for: ', key);
        }


        this.logger.debug(this.nodeName, 'removing event listener for:', key);
      }
    }
    ...
    _registerModelEvents(events) {
      for( var key in events ) {
        ...
        if( !this[methodName] ) {
          this.logger.debug(this.nodeName, 'auto-bind:', methodName+' -> '+events[key], false);
          continue;
        }

        this.logger.debug(this.nodeName, 'auto-bind:', methodName+' -> '+events[key], true);
