...
class BaseService {
  ...
  async request(options) {
    if( !options.request.uri.match(/^http/i) ) {
      options.request.uri = this.rootUrl+options.request.uri;
    }
    if( options.request.body && typeof options.request.body === 'object' ) {
    ...
    return new Promise((resolve, reject) => {
      request(options, (error, response, body) => {
        ...
        if( response.headers['content-type'] && 
            response.headers['content-type'].match(/application\/json/i) ) {
          try {
