...
class BaseService {
  ...
  call(options) {
    ...
    if( options.checkCached  ) {
      ...
      if( this.isLoaded(options.checkCached()) ) {
        return options.isLoaded();
      
      // return stored promise if loading
      } else if( this.isLoading(options.checkCached()) ) {
        if( !options.checkCached().request ) {
          throw new Error('checkCached set but no request object found', options.checkCached())
        }
        return options.checkCached().request;
      }
    }
