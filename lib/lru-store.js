...
class LruStore {
  ...
  remove(key) {
    this.logger.debug(this.name+' removing ', key);
    this.cache.delete(key);
  }

  purge() {
    this.logger.debug(this.name+' purging');
    this.cache.clear();
  }
  ...
  _clean() {
    ...
    for (let i = this.maxSize; i < keyTime.length; i++) {
      ...
      this.logger.debug(this.name+' removing expired key ', keyTime[i]);
      this.cache.delete(keyTime[i].key);
    }
  }
